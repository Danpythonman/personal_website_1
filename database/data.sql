...
VALUES
    ('automated-plant-watering','Automated Plant Watering System','Automates plant watering with a micro-controller, sensors, and actuators.','Project overview of the Automated Plant Watering System, an C++ program on an Arduino-compatible board that monitors and waters a plant.','plant_watering_system.jpg','image',6)
   ,('inspirational-website-2','Inspirational Website 2.0','An inspirational website with a daily quote and astronomy image, and a to-do list. Made with Node.js and React.','Project overview of Inspirational Website 2.0, a web app with a to-do list and a new quote and astronomy image every day.','inspirational_website_preview','video',5)
   ...
   ,('sorting-algorithm-visualizer','Sorting Algorithm Visualizer','Visualizes sorting algorithms in real time. Made with Python.','Project overview of the Sorting Algorithm Visualizer, a Python GUI that animates different sorting algorithms in real-time.','selection_sort_visualization','video',3)
   ,('tab2xml','TAB2XML','Converts text-based music tablature to playable sheet music. Made in a group project at York University using Java.','Project overview of TAB2XML, a Java GUI that converts text-based tablature to sheet music with audio playing capabilities.','tab2xml_preview','video',4)
   ,('mnist-digit-predictor','MNIST Digit Predictor','Convolutional neural network trained from scratch with PyTorch to predict hand-drawn digits.','Project overview of MNIST Digit Predictor, a Tensorflow and Flask webapp that predicts hand-drawn digits.','mnist_digit_predictor_preview','video',1)
   ,('micrograd','Micrograd - Autograd Engine for Neural Networks','A tiny auto-differentiation engine and a small neural network from scratch in Python, inspired by Andrej Karpathy''s micrograd.','Project overview of Micrograd: a minimal autograd engine and MLP in pure Python with reverse-mode auto-differentiation and backpropagation.','micrograd_big_graph2.svg','image',2)
   ,('gpt2-clone','GPT-2 Clone From Sratch','A reimplementation of OpenAI''s GPT-2 in PyTorch.','Project overview of a GPT-2 clone: transformer blocks with causal attention, GPT-2 init, AdamW, DDP training, checkpointing, and FineWeb-Edu 10B support.','gpt2_clone_preview','video',0)
;
...
VALUES
   ...
   ,(70,'micrograd', 0, 'Source Code', 'https://github.com/Danpythonman/micrograd', 'Micrograd GitHub repository', 1)
   ,(71,'gpt2-clone', 0, 'Source Code', 'https://github.com/Danpythonman/gpt-2', 'GPT-2 Clone GitHub repository', 1)
   ,(72,'gpt2-clone', 1, 'Tokenizer Source Code', 'https://github.com/Danpythonman/llm-tokenizer', 'GPT-2 Clone Tokenizer GitHub repository', 1)
;
...
CREATE TABLE `project_media` (
    ...
    `media_alt_text` varchar(512) DEFAULT NULL,
    PRIMARY KEY (`id`),
    KEY `project_url_endpoint` (`project_url_endpoint`),
...
VALUES
   ...
   ,(38,'micrograd','image','micrograd_big_graph2.svg',0,'Computation graph for a three-layer MLP','This is a computation graph produced by Micrograd for a feed-forward neural network with three layers. Each layer consists of neurons with weights and biases and tanh activations.','Computation graph for a three-layer multi-layer perceptron.')
   ,(39,'micrograd','image','micrograd_small_graph.svg',1,'Smaller computation graph','This is a much smaller computation graph. It is essentially a computation graph for a single (small) layer. It consists of two neurons with two weights and one bias, and a tanh activation function.','Computation graph for a single layer with two neurons and a tanh activation.')
   ,(40,'micrograd','image','micrograd_prediction.png',2,'Using Micrograd to predict data distribution','Generating a distribution of data of two sets of points in the shape of two moons, I trained a four-layer Micrograd-powered neural network to predict the boundary between the two sets of points. As you can see, the prediction was quite accurate (>99% accuracy).','A distribution of two colors of points in the shape of two moons with the boundary between them predicted by Micrograd.')
   ,(41,'micrograd','image','micrograd_differing_params.png',3,'Effects of different sizes','To extend the results of Micrograd''s boundary prediction, I repeated the problem a number of times, varying the number of layers in the network and the number of neurons per layer. When the number of layers or neurons per layer is too low, the prediction is too simple for the distribution of data. When it is too high, the prediction overfits the data and tracks spurious correlations. There is a Goldilocks amount that fits the data while still retaining generalizability.','A grid of plots of data boundary predictions by varying the number of layers and neurons per layer in a neural network. Too few layers or neurons per layer causes the prediction to be too simple for the data. Too many layers or neurons per layer causes the prediction to overfit the data.')
   ,(42,'gpt2-clone','video','gpt2_clone_preview',0,'GPT-2 in Action','This is my implementation of GPT-2 (loaded with the official weights from HuggingFace) running inference on the prefix: "Hello, I''m a language model,". It produces intelligible output, though not fully coherent, as it is a relatively small model.','A Python code block above text output generated by GPT-2 in real time.')
   ,(43,'gpt2-clone','image','gpt2_matrix_math.png',1,'Calculating Self-Attention via Matrix Multiplication','This is some rough work I did to understand how matrix operations can implemented functionality that would otherwise be realized through loops. By using matrix multiplication and GPUs (or SIMD processors) we can significantly speed up execution. I then used this when implementing masked self-attention in the transformer heads.','Mathematical text of matrix multiplications.')
   ,(44,'gpt2-clone','image','gpt2_position_embedding_graph.png',2,'Position Embedding Values for Random Columns','This graph shows the position embedding values (from 0 to 1024, which is the sequence length) for four randomly-chosen columns in the GPT-2 embedding matrix. Interesting patterns are observed, though they are difficult to make sense of.','Graph with four plots, each slightly different, two of which are sinusoidal.')
   ,(45,'gpt2-clone','image','gpt2_weight_matrix.png',3,'Visualization of a Random Weight Matrix','This staticky-looking image is a visualization of one of the weight matrices in GPT-2. Specifically, it is first 300 rows and columns of the first self-attention head. Lighter shades mean higher values. Notice how there are some patterns that can be observed (some areas are more smooth/less coarse). Similar to the positional embeddings graph in the previous image, it is difficult to decipher this information.','A pixelated gray square, with some areas being very coarse and others being a smooth single shade of gray.')
;
...
VALUES
   ...
   ,(286,'micrograd', 0, 'subheading', 'Project Overview')
   ,(287,'micrograd', 1, 'paragraph','This implementation mirrors <a class=\"link\" href=\"https://github.com/karpathy/micrograd\" alt=\"GitHub repository for Micrograd by Andrej Karpathy\" target=\"_blank\">Andrej Karpathy''s micrograd</a>: a scalar <code>Value</code> type tracks <code>data</code>, <code>grad</code>, the producing op, and parents. Ops build a DAG and <code>backward()</code> topologically sorts nodes and executes stored local backward closures to accumulate gradients.')
   ,(288,'micrograd', 2, 'subheading', 'Supported Operations')
   ,(289,'micrograd', 3, 'paragraph','Addition, subtraction, multiplication, division (via power -1), power by scalar, <code>tanh</code>, and <code>exp</code>. Each op defines its local gradient rule and contributes to reverse-mode accumulation.')
   ,(290,'micrograd', 4, 'subheading', 'MLP')
   ,(291,'micrograd', 5, 'paragraph','A simple MLP is built from <code>Neuron</code>, <code>Layer</code>, and <code>MultiLayerPerceptron</code> classes. Activation uses <code>tanh</code>. Parameters are lists of <code>Value</code> objects, so gradients propagate through the whole network.')
   ,(292,'micrograd', 6, 'subheading', 'Training and Loss')
   ,(293,'micrograd', 7, 'paragraph','Mean squared error over scalar outputs. Mini-batch selection is optional. After computing loss, call <code>backward()</code>, then update each parameter with SGD like <code>p.data -= lr * p.grad</code> and zero grads.')
   ,(294,'gpt2-clone', 0, 'subheading', 'Project Overview')
   ,(295,'gpt2-clone', 1, 'paragraph','PyTorch GPT-2 clone based on <a class=\"link\" href=\"https://github.com/karpathy/nanoGPT\" alt=\"GitHub repository for nanoGPT by Andrej Karpathy\" target=\"_blank\">Andrej Karpathy''s material</a>. Implements manual scaled dot-product attention and PyTorch flash attention (<code>scaled_dot_product_attention</code>) with causal masking. Uses GELU, LayerNorm pre-norm, residual connections, and tied input/output embeddings.')
   ,(296,'gpt2-clone', 2, 'subheading', 'Attention')
   ,(297,'gpt2-clone', 3, 'paragraph','Two backends: <b>manual</b> attention with explicit QK^T/√d and softmax, and <b>flash</b> attention via PyTorch for speed and memory efficiency. Autoregressive mask is a lower-triangular buffer of shape (1,1,T,T).')
   ,(298,'gpt2-clone', 4, 'subheading', 'Initialization and Optimizer')
   ,(299,'gpt2-clone', 5, 'paragraph','Weights follow GPT-2 init (Normal(0, 0.02); position embeddings 0.01). Residual branches scale down by 1/√(2·n_layer) via a flag on residual-adjacent projections. AdamW uses decoupled weight decay on weight matrices only; biases and LayerNorm weights are excluded.')
   ,(300,'gpt2-clone', 6, 'subheading', 'Pretraining and Data')
   ,(301,'gpt2-clone', 7, 'paragraph','Supports token/position embeddings for a vocab of 50,257 and context length 1024. Trains on FineWeb-Edu 10B. Uses gradient checkpointing and optional DDP for multi-GPU. Checkpoints store optimizer and model state for pausing and resuming training.')
   ,(302,'gpt2-clone', 8, 'subheading', 'HF Compatibility')
   ,(303,'gpt2-clone', 9, 'paragraph','Includes a loader to map Hugging Face GPT-2 weights into this model, transposing QKV and MLP projection matrices as needed and skipping masked bias buffers.')
   ,(304,'gpt2-clone', 10, 'subheading', 'Sampling & Eval')
   ,(305,'gpt2-clone', 11, 'paragraph','Greedy or temperature sampling over logits (B,T,V). Perplexity evaluation via average negative log-likelihood. Flash attention can be toggled per block for ablations.')
   ,(306,'gpt2-clone', 12, 'subheading', 'Tokenizer Reimplementation')
   ,(307,'gpt2-clone', 15, 'paragraph','To better understand GPT-2''s preprocessing pipeline, I reimplemented the byte pair encoding (BPE) tokenizer from scratch in a separate repository: <a class=\"link\" href=\"https://github.com/Danpythonman/llm-tokenizer\" target=\"_blank\">llm-tokenizer</a>. It reproduces GPT-2''s 50,257-token vocabulary, byte-level encoding, and BPE merges. The tokenizer works independently of Hugging Face and outputs exact token IDs for any input text.')
;
